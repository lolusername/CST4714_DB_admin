...
|---|---:|---|
| Open textbook | 15 modules; approximately 24,000 words in the second-edition source; 121-page Word/PDF review copy | Replaces disconnected readings with one coherent course narrative and formal technical reference system |
...
[Catalog of Created OER](../../course/OER_CATALOG.md). Free vendor tools and tutorials are
tracked separately in the
[Free External Resource Catalog](../../course/external_resources/INDEX.md) and are not
...
[Currentness, Pedagogy, and QA Audit](../evidence/CURRENTNESS_PEDAGOGY_QA_AUDIT.md).
...
| Independently validated both EPUB editions | Official W3C EPUBCheck 5.3.0 returned zero fatals, errors, warnings, or informational messages under EPUB 3.3 rules for the preserved first edition and the expanded second edition |
...
| Preserved and rechecked the 90-page first-edition Word export | Kept a stable comparison point rather than silently replacing the original candidate |
| Built and inspected the 121-page second-edition Word/PDF review copy | Added formal notation, 106 numbered listings, editable diagrams, current redacted cloud figures, a notation glossary, and evidence templates; every page received visual review |
| Strengthened the validator | Expanded local checks from 306 to 412, including second-edition sources, hashes, equations, listings, image descriptions, responsive controls, and all reorganized links |
| Re-ran live link QA | 414 network-enabled checks passed across 138 URLs with no 404/410; the BLS and PDF Association automated-client refusals passed independent browser review |
| Kept external work bounded | Signed-in Atlas and Supabase sessions were used only for reversible interface verification and publication-safe screenshots; no screen-reader session, CUNY login, remote push, release tag, Pressbooks import, deposit, or publication was performed |

That earlier release-engineering run ended in local commit `22790f3`, **Improve
publication format QA and metadata**. The later reorganization, cloud evidence,
and second-edition publication work remain local draft work until the instructor
explicitly authorizes a commit or push.
...
separately rather than claimed as OER. The current candidate passes 412 local
and 414 network-enabled checks. Notebook offline paths and PostgreSQL fixtures
execute, links are
audited, slide notes match transcripts, all 214 slide handouts were visually
compared, and both the preserved first-edition EPUB and the expanded
second-edition EPUB pass official W3C EPUBCheck 5.3.0 with zero messages. The
check will be repeated from the approved release commit. Known limitations,
including the weak slide-handout PDF tag semantics, are documented rather than
hidden."
...
1. Open the [course homepage](../../README.md) to establish the coherent 15-week
   sequence.
2. Open [Module 2](../../course/textbook/module_02_sql.md) to show the substantial SQL and
   relational-algebra rebuild.
3. Open the [Week 2 SQL Query Ladder](../../course/weeks/week_02/lab_01_sql_query_ladder.md) to
   show the individual, evidence-based lab format.
4. Open the [Relational and SQL Review notebook](../../course/notebooks/01_relational_sql_review.ipynb)
   to show executable instruction rather than static prose alone.
5. Open the [Week 9 NoSQL and JSON module](../../course/textbook/module_09_nosql_json.md)
   and [CSV-to-JSON lab](../../course/weeks/week_09/lab_01_csv_to_json.md) to show the bridge from
   relational thinking to multiple valid document models.
6. Open one [PowerPoint deck](../../course/weeks/week_09/week_09_nosql_models_json.pptx) and its
   [exact transcript](../../course/weeks/week_09/week_09_nosql_models_json_transcript.md) to show
   the student-facing media and complete script.
7. Open the [Implementation Guide](../../course/instructor/implementation_guide.md) to show
   that another instructor can teach the course.
8. End with the [OER Catalog](../../course/OER_CATALOG.md) and
   [External Review and Release Gates](../release/EXTERNAL_REVIEW_AND_RELEASE_GATES.md) to
