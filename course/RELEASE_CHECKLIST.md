...
- The [`oer_fellowship/` directory](../oer_fellowship/README.md) contains planning,
  evaluation, and reporting documents. Those documents are not counted as
  student course content.
- The [Free External Resource Catalog](external_resources/INDEX.md) records
...
- [x] Counts match the actual files: 15 modules, 15 weekly guides, 25 individual
  ...
  handouts.
- [x] `assignments/midterm_project.md` and `assignments/final_project.md` remain the only canonical major
...
```bash
python course/tools/validate_oer.py
```
...
```bash
python course/tools/validate_oer.py --check-urls
```
...
As of August 25, 2026, the package contains the complete planned file inventory,
including 214 authored slides with matching notes and transcripts and a fuller
second-edition textbook review system. This snapshot
becomes a release record only after all checks above are completed and a version
or tag is assigned.

### Completed Candidate Checks

- The current structural validator passed 412 local checks and 414
  network-enabled checks across 138 unique external URLs, with no confirmed 404
  or 410 result. The BLS and PDF Association pages refused the automated client
  with `403` but passed independent browser review. Live-link validation must be
  repeated from the approved release commit.
...
- The canonical 15-module second-edition Markdown text builds reproducibly as
  Word, tagged PDF, self-contained HTML, and EPUB. The 121-page review copy has
  106 numbered code listings, formal mathematical notation, editable diagrams,
  a notation glossary, evidence templates, and 14 instructional figures plus a
  separate cover image.
- The second-edition HTML contains 142 MathML elements, one navigation landmark,
  one focusable main landmark, an effective skip link, embedded styling,
  nonempty text alternatives, and no page-level horizontal overflow at tested
  1280-pixel desktop and 390-pixel mobile widths.
- SHA-256 verification passed for all four second-edition publication artifacts
  and every canonical source in the source manifest. EPUB and DOCX archive and
  XML checks passed; EPUB metadata, navigation, 142 MathML elements, and all 106
  listing labels are present. The 121-page Word draft received a complete
  page-by-page visual review; representative pages were independently rendered
  from the final tagged PDF. The second-edition EPUB passed official W3C
  EPUBCheck 5.3.0 under EPUB 3.3 rules with zero fatals, errors, warnings, or
  informational messages on August 26, 2026. The preserved first-edition EPUB
  also passed the same official validator with zero messages. Repeating
  EPUBCheck against the approved release commit remains a release gate.
...
- Offline notebook paths were executed. On August 25, a focused live check
  inspected Atlas service/access/backup boundaries and verified a reversible
  Supabase transaction without leaving a persistent object. Full credentialed
  Atlas and remote-Python Supabase notebook routes remain pending.
