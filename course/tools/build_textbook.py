...
def separate_long_heading_number(paragraph) -> bool:
    """Avoid a nearly zero-width default tab after long Pandoc section labels."""
    if not paragraph.style or not paragraph.style.name.startswith("Heading "):
        return False
    runs = paragraph.runs
    if len(runs) < 2 or not re.fullmatch(r"\d+(?:\.\d+)+", runs[0].text):
        return False
    if len(runs[0].text) < 5 or runs[1].text != "\t":
        return False
    runs[1].text = " "
    return True
...
def polish_docx(path: Path) -> None:
    ...
    for index, paragraph in enumerate(paragraphs):
        separate_long_heading_number(paragraph)
