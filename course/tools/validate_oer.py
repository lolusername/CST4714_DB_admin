...
FELLOWSHIP = ROOT.parent / "oer_fellowship"
LEGACY_PUBLICATION = ROOT.parent / "archive" / "oer_first_edition" / "publication"
...
def check_structure(report: Report) -> None:
    ...
    weeks = sorted(path for path in weeks_root.glob("week_*") if path.is_dir())
    expected_weeks = [weeks_root / f"week_{number:02d}" for number in range(1, 16)]
    report.require(weeks == expected_weeks, "15 weekly directories are present")

    modules = sorted((ROOT / "textbook").glob("module_*.md"))
    labs = sorted(ROOT.glob("weeks/week_*/lab_*.md"))
    ...
    lab_counts = [1, 2, 2, 2, 2, 2, 2, 1, 1, 2, 2, 2, 1, 1, 1]
    for week, expected in zip(expected_weeks, lab_counts):
        week_labs = sorted(week.glob('lab_*.md'))
        report.require(len(week_labs) == expected,
                       f"{week.name} has its {expected} assigned individual lab(s)")
        guide = read_text(week / 'README.md') if (week / 'README.md').is_file() else ''
        for lab in week_labs:
            report.require(lab.name in guide, f"weekly guide links its lab: {relative(lab)}")
    report.require(len(labs) == sum(lab_counts), f"{sum(lab_counts)} individual lab files are present")
    notebook_names = {
        '01_relational_sql_review.ipynb', '02_postgres_transactions_locks.ipynb',
        '03_postgres_backup_restore.ipynb', '04_atlas_mql_modeling.ipynb',
        '05_mongodb_logical_recovery.ipynb', '06_public_data_capacity_integration.ipynb',
        '07_aggregation_validation.ipynb',
    }
    report.require({path.name for path in notebooks} == notebook_names,
                   '7 named educational notebooks are present')
    catalog = read_text(ROOT / 'OER_CATALOG.md')
    report.require('| Individual in-class labs | 24 |' in catalog,
                   'catalog lab count agrees with the assigned files')
    report.require('| Educational notebooks | 7 |' in catalog,
                   'catalog notebook count agrees with the named files')
    ...
    required_root_files = {
        "README.md",
        "OER_CATALOG.md",
        "external_resources/INDEX.md",
        "ATTRIBUTIONS.md",
        "LICENSE.md",
        "CITATION.cff",
        "course_map.md",
        "syllabus.md",
        "assignments/midterm_project.md",
    ...
    report.require(
        (FELLOWSHIP / "evidence/CURRENTNESS_PEDAGOGY_QA_AUDIT.md").is_file(),
        "currentness and pedagogy audit record exists",
    )
    accessibility_report = (
        FELLOWSHIP / "evidence/STANDALONE_HTML_ACCESSIBILITY_QA.md"
    )
    report.require(
    ...
    if accessibility_report.is_file():
        ...
        fellowship_index = read_text(FELLOWSHIP / "README.md")
        report.require(
    ...
    format_report = FELLOWSHIP / "evidence/PUBLICATION_FORMAT_QA.md"
    report.require(
...
def check_json_csv_and_notebooks(report: Report) -> None:
    ...
    for path in notebooks:
        ...
        report.require(
            'File > Upload notebook' in source_text
            and 'colab.research.google.com/github/lolusername' not in source_text,
            f"private-draft notebook gives a usable download/upload path: {path.name}",
        )
        code_text = "\n".join(
    ...
    disabled_markers = {
        ...
        "07_aggregation_validation.ipynb": "USE_ATLAS = False",
    }
    report.require(
    ...
    report.require(
        ...
        and 'database.drop_collection("tickets")' in atlas_notebooks[0]
        and 'client.close()' in atlas_notebooks[0]
        and 'uuid4().hex[:8]' in atlas_notebooks[0]
        and 'source_database.drop_collection("tickets")' in atlas_notebooks[1]
        and 'restore_database.drop_collection("tickets")' in atlas_notebooks[1]
        and 'EXPORT_FILE.unlink(missing_ok=True)' in atlas_notebooks[1]
        and 'uuid4().hex[:8]' in atlas_notebooks[1]
        and 'atlas_client.drop_database(atlas_database_name)' in atlas_notebooks[2]
        and 'sql.Identifier(postgres_schema)' in atlas_notebooks[2]
        and 'database.drop_collection("tickets")' in atlas_notebooks[3]
        and 'client.close()' in atlas_notebooks[3],
        "cloud notebook source declares disposable cleanup; runtime checks remain separate",
    )
    aggregation_code = atlas_notebooks[3]
    recovery_code = atlas_notebooks[1]
    report.require(
        all(marker in recovery_code for marker in (
            'expected_canonical', 'saved_by_id[DAMAGED_TICKET]',
            'unique_ticket_id', 'NOT a result from this runtime',
            '"Date stored as text"', '"Missing date"',
        )),
        "recovery notebook declares a typed-value repair and distinct rule tests",
    )
    report.require(
        'tls=True, tlsInsecure=False' in recovery_code
        and 'runtime_ip + "/32"' in recovery_code
        and 'finally:\n        mongodb_uri = None' in recovery_code,
        "recovery notebook declares verified TLS, one runtime IP, and URI cleanup",
    )
    report.require(
        all(marker in aggregation_code for marker in (
            '"Invalid status"', '"Date stored as text"', '"Missing date"',
            '"opened_at" in validator["$jsonSchema"]["required"]',
            'NOT a result from this runtime',
        )),
        "aggregation notebook declares status/date counterexamples and labels its local trace",
    )
    report.require(
        'tls=True, tlsInsecure=False' in aggregation_code
        and 'runtime_ip + "/32"' in aggregation_code
        and 'finally:\n        mongodb_uri = None' in aggregation_code,
        "aggregation notebook declares verified TLS, a single-IP rule, and URI-variable cleanup",
    )
...
def check_publication(report: Report) -> None:
    ...
    if metadata_date_match and catalog_date_match:
        try:
            ...
            dates_match = False
    report.require(
        metadata_date_match is not None
        and metadata_date_match.group(1) == "2026-08-09"
        and catalog_date_match is not None,
        "preserved first-edition publication date and current catalog date are recorded",
    )


def check_textbook_publication(report: Report) -> None:
    publication = ROOT / "textbook" / "publishing"
    exports = publication / "exports"
    stem = "operating_cloud_databases_second_edition_draft"
    outputs = {
        suffix: exports / f"{stem}.{suffix}"
        for suffix in ["docx", "pdf", "html", "epub"]
    }
    required = [
        publication / "README.md",
        publication / "STYLE_GUIDE.md",
        publication / "book_metadata.yaml",
        publication / "front_matter.md",
        ...
        publication / "cover_2e.svg",
        publication / "cover_2e.png",
        exports / "SHA256SUMS.txt",
        exports / "SOURCE_MANIFEST.json",
        *outputs.values(),
    ]
    for path in required:
        report.require(path.is_file(), f"second-edition artifact exists: {relative(path)}")
    if not all(path.is_file() for path in required):
        return

    modules = sorted((ROOT / "textbook").glob("module_*.md"))
    report.require(len(modules) == 15, "second edition contains all 15 editable modules")
    unlabeled_fences = []
    for module in modules:
        in_fence = False
        for line_number, line in enumerate(read_text(module).splitlines(), start=1):
            if not line.startswith("```"):
                continue
            if not in_fence and line.strip() == "```":
                unlabeled_fences.append(f"{relative(module)}:{line_number}")
            in_fence = not in_fence
    report.require(
        not unlabeled_fences,
        "every second-edition code fence declares its language",
    )

    metadata = read_text(publication / "book_metadata.yaml")
    report.require(
        'version: "2.0.0-draft"' in metadata
        and bool(re.search(r'date: "\d{4}-\d{2}-\d{2}"', metadata)),
        "second-edition metadata identifies the unpublished draft and date",
    )

    source_manifest = json.loads(read_text(exports / "SOURCE_MANIFEST.json"))
    source_entries = source_manifest.get("canonical_sources", [])
    ...
    for entry in source_entries:
        ...
        if not source.is_file() or hashlib.sha256(source.read_bytes()).hexdigest() != entry.get("sha256"):
            ...
            break
    report.require(
        len(source_entries) == 24 and source_hashes_match,
        "second-edition manifest identifies and verifies all 24 canonical book sources",
    )
    supporting = source_manifest.get("supporting_sources", [])
    report.require(
        bool(supporting) and all(
            (ROOT / entry["path"]).is_file()
            and hashlib.sha256((ROOT / entry["path"]).read_bytes()).hexdigest() == entry["sha256"]
            for entry in supporting
        ),
        "book manifest verifies supporting images, styles, metadata, filters, and builder",
    )

    pandoc = shutil.which("pandoc")
    if not pandoc:
        report.fail("Pandoc is required to compare current source math/listing/image counts with exports")
        return
    parsed = subprocess.run(
        [pandoc, "--from=markdown+fenced_divs+tex_math_dollars+pipe_tables+implicit_figures+link_attributes+bracketed_spans+header_attributes", "--to=json",
         *[str(ROOT / entry["path"]) for entry in source_entries]],
        text=True, capture_output=True, check=True,
    )
    source_counts = {"Math": 0, "CodeBlock": 0, "Image": 0}
    def count_nodes(node):
        if isinstance(node, dict):
            ...
            for value in node:
                count_nodes(value)
    count_nodes(json.loads(parsed.stdout))
    expected_math = source_counts["Math"]
    expected_listings = source_counts["CodeBlock"]
    expected_images = source_counts["Image"]
    report.require(min(source_counts.values()) > 0, "current textbook sources include math, code, and images")

    expected_hashes: dict[str, str] = {}
    for line in read_text(exports / "SHA256SUMS.txt").splitlines():
        if line.strip():
            checksum, filename = line.split(maxsplit=1)
            expected_hashes[filename.strip()] = checksum
    report.require(
        len(expected_hashes) == 4
        and all(
            expected_hashes.get(path.name) == hashlib.sha256(path.read_bytes()).hexdigest()
            for path in outputs.values()
        ),
        "all four second-edition exports match the checksum manifest",
    )

    html_text = read_text(outputs["html"])
    companion_hrefs = re.findall(r'href="(\.\./[^"#]+)', html_text)
    report.require(
        bool(companion_hrefs) and all(
            (exports / urllib.parse.unquote(href)).resolve().is_file()
            for href in companion_hrefs
        ),
        "exported HTML companion links resolve from the exported file's actual location",
    )
    companion_index = read_text(publication / "appendix_b_technical_templates.md")
    named_companions = re.findall(r"`((?:notebooks|datasets|weeks)/[^`]+)`", companion_index)
    report.require(
        len(named_companions) >= 9 and all((ROOT / name).is_file() for name in named_companions),
        "EPUB companion index names existing course files",
    )
    report.require(
        len(re.findall(r"<math\b", html_text)) == expected_math
        and len(re.findall(r"Listing\s+\d+", html_text)) == expected_listings
        and all(f'>{number}</span>' in html_text for number in range(1, 16)),
        f"second-edition HTML preserves all {expected_math} source math elements, {expected_listings} listings, and 15 chapters",
    )
    report.require(
        html_text.count('<nav id="TOC"') == 1
        and html_text.count('<main id="main-content" tabindex="-1">') == 1
        and '<a class="skip-link" href="#main-content">Skip to main content</a>' in html_text
        and "overflow-wrap: anywhere" in html_text
        and "box-sizing: border-box" in html_text,
        "second-edition HTML contains semantic navigation and responsive overflow controls",
    )
    images = re.findall(r"<img\b[^>]*>", html_text)
    report.require(
        len(images) == expected_images and all(re.search(r'\balt="[^"]+"', image) for image in images),
        f"all {expected_images} source images appear in HTML with nonempty text alternatives",
    )

    for path, label in [(outputs["epub"], "second-edition EPUB"), (outputs["docx"], "second-edition Word")]:
        try:
            with zipfile.ZipFile(path) as archive:
                report.require(archive.testzip() is None, f"{label} archive passes integrity check")
        except zipfile.BadZipFile as exc:
            report.fail(f"invalid {label} archive: {exc}")

    try:
        with zipfile.ZipFile(outputs["epub"]) as archive:
            epub_text = "".join(
            ...
            report.require(
                len(re.findall(r"<math\b", epub_text)) == expected_math
                and len(re.findall(r"Listing\s+\d+", epub_text)) == expected_listings
                and "EPUB/nav.xhtml" in archive.namelist(),
                "second-edition EPUB preserves MathML, listing labels, and navigation",
            )
    except zipfile.BadZipFile as exc:
        report.fail(f"second-edition EPUB structure could not be inspected: {exc}")

    try:
        with zipfile.ZipFile(outputs["docx"]) as archive:
            document_xml = archive.read("word/document.xml")
            report.require(
                len(ET.fromstring(document_xml).findall(".//{http://schemas.openxmlformats.org/officeDocument/2006/math}oMath")) == expected_math
                and len(re.findall(rb"Listing\s+\d+", document_xml)) == expected_listings
                and len(ET.fromstring(document_xml).findall(".//{http://schemas.openxmlformats.org/drawingml/2006/wordprocessingDrawing}docPr")) == expected_images + 1
                and document_xml.count(b'descr=') >= expected_images + 1,
                "second-edition Word preserves equations, listing labels, and image descriptions",
            )
    except (KeyError, zipfile.BadZipFile) as exc:
        report.fail(f"second-edition Word structure could not be inspected: {exc}")

    with outputs["pdf"].open("rb") as handle:
        report.require(
            handle.read(5) == b"%PDF-",
            "second-edition PDF has a valid PDF signature",
        )
...
def check_decks(report: Report) -> None:
    ...
    decks = sorted(ROOT.glob("weeks/week_*/*.pptx"))
    ...
    catalog = read_text(ROOT / "OER_CATALOG.md")
    declared = re.search(r"containing (\d+) authored slides", catalog)
    report.require(
        declared is not None and total_slides == int(declared.group(1)),
        f"all {total_slides} authored slides match the declared catalog count",
    )
...
def check_labs_and_assignments(report: Report) -> None:
    ...
    for lab in labs:
        ...
        report.require(
            len(re.findall(r"^(?:## (?:\d+\. )?Submit(?: One Thing)?\s*$|\*\*Submit:\*\*)", text, re.MULTILINE)) == 1,
            f"lab has one explicit submission section: {relative(lab)}",
        )

    weekly_readmes = sorted(ROOT.glob("weeks/week_*/README.md"))
    ...
    for path in weekly_readmes:
        ...
        report.require(
            ...
            f"weekly guide has at most one optional industry extension: {relative(path)}",
        )
        report.require(
            not optional_headings or "This activity is optional, ungraded, and does not add a submission."
            in weekly_text,
            f"optional extension does not change required workload: {relative(path)}",
        )
        report.require(
            "**Before Day 1:**" in weekly_text and "**Before Day 2:**" in weekly_text,
            f"weekly guide identifies readings for both meetings: {relative(path)}",
        )

    final_text = read_text(ROOT / "assignments/final_project.md")
    report.require(
    ...
    week_10 = read_text(ROOT / "weeks/week_10/README.md")
    live_slug = "/modeling-data-relationships/lesson-3-modeling-one-to-one/learn"
    student_slug = "/modeling-data-relationships/lesson-4-modeling-one-to-many/practice?page=1"
    report.require(
        live_slug in week_10 and student_slug in week_10,
        "Week 10 links distinct instructor-live and individual MongoDB activities",
    )
    report.require(
        "different lessons and practice activities" in week_10
        and "do not repeat the instructor" in week_10
        and student_slug in read_text(ROOT / "weeks/week_10/lab_02_document_model.md"),
        "Week 10 distinguishes specific instructor/student lessons and links the student's Practice",
    )

    response = read_text(ROOT / "weeks/week_11/lab_02_sort_and_case_response.md")
    report.require(
...
def check_oer_boundary(report: Report) -> None:
    ...
    external = read_text(ROOT / "external_resources/INDEX.md")
    fellowship = read_text(FELLOWSHIP / "README.md")
    readme = normalize_prose(read_text(ROOT / "README.md"))
    ...
    report.require(
        "deliberately separate" in fellowship.lower()
        and "catalog of created oer" in fellowship.lower(),
        "fellowship planning is explicitly separate from created OER evidence",
    )
    report.require(
        not (FELLOWSHIP / "OER_CATALOG.md").exists(),
        "the authoritative OER catalog is not duplicated inside the fellowship plan",
    )
    report.require(
        "PDF handout" in readme
        and "convenience copies" in readme
        and "structured transcript" in readme
        and "reading-order and accessibility review" in readme
        and "alone does not establish accessibility" in readme,
...
def main() -> int:
    ...
    check_textbook_publication(report)
    check_decks(report)
