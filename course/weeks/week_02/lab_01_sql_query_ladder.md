...
For a cloud-free path, download the
[SQL notebook](../../notebooks/01_relational_sql_review.ipynb), upload it through
**File > Upload notebook** in [Colab](https://colab.research.google.com/), and run
through **Use the complete Week 2 lab dataset**. That section replaces the
smaller demonstration data. Use `con.sql("""YOUR QUERY""").show()` to run a SELECT.
